...
/**
 * @title IDOSale
 * @dev Fixed-price tiered token sale contract for PULSE IDO
 *
 * Features:
 ...
 */
contract IDOSale is Ownable, ReentrancyGuard, Pausable {
    ...
    struct SaleConfig {
        uint256 startTime;
        uint256 endTime;
        uint256 tokenPrice; // Price per PULSE in wei (e.g., 0.0015 USD = 1500000000000000 wei if 1 ETH = $1000)
    ...
    struct UserContribution {
        ...
        uint256 totalTokens; // Total PULSE allocated
    ...
    IERC20 public immutable pulseToken; // PULSE token contract
    IERC20 public immutable usdcToken; // USDC payment token
    ...
    function buyTokensWithUSDC(
        ...
        TierConfig storage tier = tiers[_tier];
        // USDC has 6 decimals, PULSE has 18 decimals
    ...
    function claimTGE() external nonReentrant {
        ...
        pulseToken.safeTransfer(msg.sender, tgeAmount);
    ...
    function finalizeSale() external onlyOwner {
        ...
        if (totalRaised < saleConfig.softCap) {
            ...
            if (vestingContract != address(0)) {
                ...
                pulseToken.safeTransfer(vestingContract, vestingAmount);
            }
        }
    ...
    function emergencyWithdraw() external onlyOwner {
        ...
        if (pulseBalance > 0) {
            pulseToken.safeTransfer(owner(), pulseBalance);
        }
    }
